...
#Install EAP
RUN java -jar $INSTALLERS_DIR/jboss-eap-7.0.0-installer.jar $SUPPORT_DIR/eap.xml -variablefile $SUPPORT_DIR/eap.xml.variables

#Patch EAP
WORKDIR $JBOSS_HOME
#RUN bin/standalone.sh --admin-only 2>&1 > /dev/null & sleep 4 && bin/jboss-cli.sh -c "patch apply $INSTALLERS_DIR/jboss-eap-7.0.2-patch.zip,shutdown"
